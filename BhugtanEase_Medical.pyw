...
#
# Lekin agar app "Program Files" jaise protected folder me install hai
# (jahan normal user write nahi kar sakta), to DB ko us location pe nahi,
# balki user ke AppData (%LOCALAPPDATA%) me banao -> ye standard Windows
# practice hai aur har user ke paas wahan write access hota hai.
def _resolve_db_path():
    ...
    candidate = os.path.join(exe_dir, "bhugtanease.db")

    # Check karo ki exe_dir me likhne ki permission hai ya nahi
    writable = False
    try:
        test_path = os.path.join(exe_dir, ".__write_test__.tmp")
    ...
    # Fallback: user ke AppData folder me (hamesha writable hota hai)
    app_data = os.environ.get("LOCALAPPDATA") or os.environ.get("APPDATA") or os.path.expanduser("~")
    app_dir = os.path.join(app_data, "BhugtanEase Medical")
    os.makedirs(app_dir, exist_ok=True)
    return os.path.join(app_dir, "bhugtanease.db")
...
def init_db():
    ...
    for sql in [
        ...
        # ── Pack Size: strip me kitne tablets — 1 matlab loose/no conversion ──
        "ALTER TABLE products ADD COLUMN pack_size REAL DEFAULT 1",
    ]:
...
class AppWin:
    ...
    def _pur_add_item(self):
        ...
        # ── Pack Size — seedha "Packing" field se (jo pehle se bharte ho, jaise 10) ──
        # Agar number hai, wahi 1 strip ke tablets maan liya jayega. Extra step nahi.
        pack_size = 1.0
        if packing_str:
            try:
                _ps = float(packing_str)
                if _ps > 0: pack_size = _ps
            except:
                pack_size = 1.0
        if pack_size == 1.0:
            # Is baar Packing khaali chhoda ho to, pichhli purchase mein jo save hua tha wahi use karo
            try:
                prow = get_db().execute("SELECT pack_size FROM products WHERE name=?", (prod,)).fetchone()
                if prow and prow["pack_size"]:
                    ps2 = float(prow["pack_size"])
                    if ps2 > 0: pack_size = ps2
            except:
                pass
        stock_qty      = round(qty * pack_size, 3)
        stock_free_qty = round(free_qty * pack_size, 3)
        stock_rate     = round(rate / pack_size, 4) if pack_size else rate
        ...
        # Sale Rate bhi strip-wise entered ho sakta hai (jaise 10 tablet ki strip ka MRP-based
        # sale price) — Purchase Rate ki tarah usko bhi pack_size se divide karo taaki
        # Products Master ka sale_rate hamesha per-tablet (base unit) consistent rahe.
        stock_sale_rate = round(sale_rate / pack_size, 4) if (pack_size and sale_rate) else sale_rate
        ...
        self._pur_items.append({
            ...
            "sale_rate"  : sale_rate,
            "stock_sale_rate": stock_sale_rate,
            "disc"       : disc_pct,
            "disc_amt"   : disc_amt,
        })
        self._pur_render()
    ...
    def _pur_render(self):
        ...
        for i,it in enumerate(self._pur_items):
            ...
            if it.get("free_qty"):
                qty_show = f"{it['qty']}+{it['free_qty']}F"
            if it.get("pack_size", 1) and it["pack_size"] != 1:
                qty_show = f"{qty_show} (={it.get('stock_qty',it['qty'])+it.get('stock_free_qty',0):g} pcs)"
            ...
            for it in self._pur_items:
                ...
                if not existing:
                    c.execute(
                        ...
                        "mrp, gst_percent, opening_stock, unit, barcode, track_expiry, pack_size) "
                        "VALUES(?,?,?,?,?,?,0,?,?,0,?)",
                        (
                            it["product"],
                            it.get("hsn", ""),
                            it.get("stock_rate", it["rate"]),
                            it.get("stock_sale_rate", 0) or it.get("stock_rate", it["rate"]),
                            it.get("exp_mrp", 0),
                            it["gst"],
                            it.get("unit", "Strip"),
                            it.get("barcode", ""),
                    ...
                    # Product exist karta hai — sab fields update karo (pack_size sirf tab update
                    # hoga jab is purchase mein Packing bharke number diya ho, warna purana rahega)
                    if it.get("pack_size", 1) != 1:
                        c.execute(
                            ...
                            "pack_size=? "
                            "WHERE name=?",
                            (it.get("stock_rate", it["rate"]), it.get("exp_mrp",0),
                             it.get("hsn",""),
                             it.get("stock_sale_rate",0), it.get("stock_sale_rate",0),
                             it.get("pack_size",1),
                             it["product"])
                        )
                    else:
                ...
                # ── FIFO Layer add karo (Qty + Free Qty dono stock mein, pack-size multiply ho chuka hai) ──
                fifo_add_layer(c, it["product"], date, bill_no,
                               it.get("batch_no",""),
                               it.get("stock_qty",it["qty"])+it.get("stock_free_qty",it.get("free_qty",0)),
                               it.get("stock_rate", it["rate"]))
                ...
                if it.get("expiry_date"):
                    ...
                    if same:
                        c.execute(
                            ...
                            (it.get("stock_qty",it["qty"])+it.get("stock_free_qty",it.get("free_qty",0)),
                             it.get("stock_rate", it["rate"]), it.get("exp_mrp",0), same["id"])
                        )
                    else:
                        c.execute(
